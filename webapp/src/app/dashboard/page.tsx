...
export default async function DashboardPage() {
  ...
  // The accent goes on whichever signal is actually real right now — clicks
  // (the rarer, more meaningful one) over opens over nothing — instead of
  // always painting the same card regardless of whether it has anything to say.
  const heroMetric = clicks.length > 0 ? "clicks" : opens.length > 0 ? "opens" : null;
  ...
  return <>
    ...
    <section className="metric-row">
      ...
      <article className={heroMetric === "opens" ? "metric-card accent" : "metric-card"}><span>Confirmed opens</span><strong>{opens.length}</strong><small>{opens.length ? `${Math.round((opens.length / Math.max(1, emails.length)) * 100)} per 100 sends` : "Waiting for your first signal"}</small></article>
      <article className={heroMetric === "clicks" ? "metric-card accent" : "metric-card"}><span>Link clicks</span><strong>{clicks.length}</strong><small>{clicks.length ? "Action taken by recipients" : "No clicks yet"}</small></article>
    </section>
    <section className="dashboard-grid">
      <article className="panel activity-panel"><div className="panel-heading"><div><p className="eyebrow">Right now</p><h2>Latest activity</h2></div><span className="status-pill"><i className="pulse-dot" /> Live</span></div>
